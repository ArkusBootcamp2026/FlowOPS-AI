 ...
 * @param input - Member data to update
 * @returns The updated member or null if there's an error
 */
export async function updateMember(id: string, input: UpdateMemberInput): Promise<MemberFromDB | null> {
  try {
    // Validate required fields
    if (!input.fullName || !input.email || !input.role) {
      throw new Error('fullName, email, and role are required fields')
    ...
    // Check if the email is being changed and if it already exists for another member
    if (input.email.trim().toLowerCase() !== existingMember.email.toLowerCase()) {
    ...
    // 2. Insert new skills if there are any
    if (input.skillIds.length > 0) {
      // Validate that all skills exist in the database
      const { data: existingSkills, error: skillsCheckError } = await supabase
        .from('Skills')
